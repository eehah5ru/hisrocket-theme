...
class ThemeOptions {
	...
	function ThemeOptions() {
		...
		setOptionDefault('hrsi_menu', '');
		setOptionDefault('hrsi_logo', '');
		setOptionDefault('hrsi_colorbox', false);
		setOptionDefault('hrsi_cbstyle', 'style3');
		setOptionDefault('hrsi_zpsearchcount', 2);
		setOptionDefault('hrsi_finallink', 'nolink');
	...
	function getOptionsSupported() {
		return array(	
			...
			gettext('Colorbox Style') => array('key' => 'hrsi_cbstyle', 'type' => OPTION_TYPE_CUSTOM, 'desc' => gettext('Select the Colorbox style you wish to use (examples on the colorbox site).')),
			gettext('Disable MetaData Display?') => array('key' => 'hrsi_disablemeta', 'type' => OPTION_TYPE_CHECKBOX, 'desc' => gettext('Check to disable the metadata (EXIF,IPTC) display on the image page.')),
			gettext('Social Networks Sharing Links?') => array('key' => 'hrsi_social', 'type' => OPTION_TYPE_CHECKBOX, 'desc' => gettext('Check to enable the social sharing buttons on various pages (facebook, twitter, etc).')),
			gettext('Album Thumb Size') => array('key' => 'hrsi_album_thumb_size', 'type' => OPTION_TYPE_TEXT, 'desc' => gettext('Select the size of album thumbs. The thumb size above is for image thumbs. Album thumbs will also take on your selection of cropping or not.  A good size for albums is 158 (3 per row), and a good size above for image thumbs is 113 (4 per row). Make sure you also set the number of thumbs per row for images and albums above! ')),
			gettext('Menu Name of Menu Manager') => array('key' => 'hrsi_menu', 'type' => OPTION_TYPE_TEXT, 'desc' => gettext('If you use the menu manager plugin, enter the name of the menu you would like to use.  Make sure you create the menu in the menu manager backend! Or leave blank to use the theme menu.')),
			gettext('ZenPage Search Results') => array('key' => 'hrsi_zpsearchcount', 'type' => OPTION_TYPE_TEXT, 'desc' => gettext('If using Zenpage, enter the number of search results to display for each news and pages.  Default is 2 (4 total possible).')),
			gettext('Logo Image') => array('key' => 'hrsi_logo', 'type' => OPTION_TYPE_TEXT, 'desc' => gettext('Enter an image file located in the themes image directory, including extension, to use as an image logo. Or leave blank to use a text representation of your Gallery name.  As an example there is a logo.gif image in the images directory.'))
