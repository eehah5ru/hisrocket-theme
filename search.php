...
if ($zenpage && !isArchive()) {
	$numpages = getNumPages(); if ($numpages > $hrv_zpsearchcount) $numpages = $hrv_zpsearchcount;
	$numnews = getNumNews(); if ($numnews > $hrv_zpsearchcount) $numnews = $hrv_zpsearchcount;
				...
				<div id="main"<?php if ($hrv_switch) echo ' class="switch"'; ?>>
					<div id="albums-wrap">
						...
						<div class="album-maxspace">
							<a class="thumb-link" href="<?php echo html_encode(getAlbumLinkURL());?>" title="<?php echo shortenContent(getBareAlbumDesc(),300,'...'); ?>">
								<?php if ($hrv_thumb_crop) {
								printCustomAlbumThumbImage(getAnnotatedAlbumTitle(),null,$hrv_album_thumb_size,$hrv_album_thumb_size,$hrv_album_thumb_size,$hrv_album_thumb_size);
								} else {
								printCustomAlbumThumbImage(getAnnotatedAlbumTitle(),$hrv_album_thumb_size);
								} ?>
					...
					<div id="thumbs-wrap">
						...
						<div class="thumb-maxspace">
							...
							<?php if (($hrv_colorbox) && ($cb) && (!isImageVideo())) { ?>
							<div class="cblinks">
					...
					if ($numpages > 0) {
					$zpc = 0;
					while (($zpc < $hrv_zpsearchcount) && (next_page())) {
						$zpc++; $c++; ?>
					...
					if ($numnews > 0) {
					$zpc = 0;
					while (($zpc < $hrv_zpsearchcount) && (next_news())) {
						$zpc++; $c++; ?>
						<div class="news-truncate"> 
