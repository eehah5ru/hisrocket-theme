...
if ($zenpage && !isArchive()) {
	$numpages = getNumPages(); if ($numpages > $hrvb_zpsearchcount) $numpages = $hrvb_zpsearchcount;
	$numnews = getNumNews(); if ($numnews > $hrvb_zpsearchcount) $numnews = $hrvb_zpsearchcount;
...
if (!empty($searchdate)) {
	...
	$searchwords .= $searchdate;
			...
			<div id="content">
				<div id="main"<?php if ($hrvb_switch) echo ' class="switch"'; ?>>
					<div id="albums-wrap">
						...
						<div class="album-maxspace">
							<a class="thumb-link" href="<?php echo html_encode(getAlbumLinkURL());?>" title="<?php echo shortenContent(getBareAlbumDesc(),300,'...'); ?>">
								<?php if ($hrvb_thumb_crop) {
								printCustomAlbumThumbImage(getAnnotatedAlbumTitle(),null,$hrvb_album_thumb_size,$hrvb_album_thumb_size,$hrvb_album_thumb_size,$hrvb_album_thumb_size);
								} else {
								printCustomAlbumThumbImage(getAnnotatedAlbumTitle(),$hrvb_album_thumb_size);
								} ?>
					...
					<div id="thumbs-wrap">
						...
						<div class="thumb-maxspace">
							...
							<?php if (($hrvb_colorbox) && ($cb) && (!isImageVideo())) { ?>
							<div class="cblinks">
					...
					if ($numpages > 0) {
					$zpc = 0;
					while (($zpc < $hrvb_zpsearchcount) && (next_page())) {
						$zpc++; $c++; ?>
					...
					if ($numnews > 0) {
					$zpc = 0;
					while (($zpc < $hrvb_zpsearchcount) && (next_news())) {
						$zpc++; $c++; ?>
						<div class="news-truncate"> 
				...
				<div id="sidebar"<?php if ($hrvb_switch) echo ' class="switch"'; ?>>
